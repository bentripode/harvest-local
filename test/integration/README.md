...
| `cap-variants.test.ts` | `record_order_revenue` across the three cap bases — Colorado counted per product (two products at $6,000 each is fine; one at $11,000 pauses), Virginia tallying only the capped axis, Minnesota's licensing threshold recorded once and never pausing, and the idempotency guard still holding |
| `label-disclosure.test.ts` | `product_label_disclosure` — an anonymous buyer can read the label, including the producer address that RLS otherwise hides from them, but nothing beyond the label fields; drafts disclose nothing; the Texas flag and statutory disclaimer come through |
| `program-review.test.ts` | verifying a program — an admin can correct and stamp `verified_at`, the gates read the correction immediately, a seller cannot verify, and the cap-basis / cap-category / gate-value constraints still refuse nonsense |
